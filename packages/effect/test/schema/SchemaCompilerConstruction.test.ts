...
describe("Schema compiler construction", { concurrent: false }, () => {
  ...
  it("keeps selective compilation below an interpreted Suspend", () => {
    ...
    Object.defineProperty(child.ast, "getParser", {
      value() {
        throw new Error("interpreted child")
      }
    })
  ...
  it("does not compile validators when only construction is used", () => {
    ...
    const emit = vi.spyOn(Codegen, "emitValidate")
    try {
  ...
  it("does not compile construction when only decoding is used", () => {
    const schema = Schema.Struct({ a: Schema.String })
    const emit = vi.spyOn(Codegen, "emitComposedObject")
    try {
      SchemaJITCompiler.enable(schema.ast)
  ...
  it("prepares selective Declaration parameters for public decoders in the callback", () => {
    ...
    Object.defineProperty(child.ast, "getParser", {
      value() {
        throw new Error("interpreted Declaration parameter")
      }
    })
    assert.deepStrictEqual(SchemaParser.make(schema)(new Set([{ value: "a" }])), new Set([{ value: "a" }]))
    assert.strictEqual(Registry.resolve(child.ast).origin, "installed")
  })
  ...
  it("does not restart a parent if compilation fails after a default", () => {
    ...
    const emit = Codegen.emitComposedObject
    const failure = vi.spyOn(Codegen, "emitComposedObject").mockImplementation((ast) => {
      if (ast === childAST) {
  ...
  for (const operation of ["make", "decode"] as const) {
    it(`keeps the other operation compiled after ${operation} compilation fails`, () => {
      const schema = Schema.Struct({ a: Schema.String })
      SchemaJITCompiler.enable(schema.ast)
      const failed = vi.spyOn(Codegen, operation === "make" ? "emitComposedObject" : "emitValidate")
      ...
      try {
        ...
        Object.defineProperty(schema.ast, "getParser", {
          value() {
            throw new Error("other operation fell back")
          }
        })
  ...
  it("runs generated Struct construction without preparing an interpreter", () => {
    const schema = Schema.Struct({
      a: Schema.String,
    ...
    Object.defineProperty(schema.ast, "getParser", {
      value() {
        throw new Error("interpreted Struct")
      }
    })
    const make = SchemaParser.make(schema)
