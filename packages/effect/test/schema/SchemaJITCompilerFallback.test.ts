...
describe("Schema JIT compilation fallback", () => {
  ...
  for (const phase of ["select", "emitValidate"] as const) {
  ...
  it("recovers composed decoder generation without repeating transformations", () => {
    ...
    const failure = vi.spyOn(Codegen, "emitComposedObject").mockImplementation(() => {
      throw new Error("composed decoder generation failed")
    })
    try {
      const decode = SchemaParser.decodeUnknownSync(schema)
      ...
      strictEqual(failure.mock.calls.length, 1)
    } finally {
      failure.mockRestore()
    }
  })

  it("recovers a lazy local checkpoint without replaying its encoding or middleware", () => {
    let transformations = 0
    ...
    try {
      ...
      strictEqual(transformationsAtFailure, 1)
